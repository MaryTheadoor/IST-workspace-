...
- Section 5: Simulation results -- the 51-phase computational program from static-$\varphi$ falsification to the plonk-scale substrate and the true incommensurate spectrum
- Section 6: Quantum-scale derivations -- the half-integer twist and the particle spectrum, culminating in the derived $\theta = 1/2$, the Fibonacci Standard Model, and the 6π⁵ duality
...
The $\varphi$-emergence claim rests on a code-verified computational program (51 phases, 600 automated tests; Python 3.14, numpy/scipy/numba, all reproducible). The arc is: (i) falsify static $\varphi$ (Phases 1--4); (ii) demonstrate the anti-resonance attractor (Phases 6--9); (iii) establish dynamical RG convergence and fold-density feedback (Phases 10--14); (iv) close quantitative gaps (Phase 15); (v) test against real data (Phases 16--17); (vi) implement the plonk-scale substrate (Phases 23--24); (vii) validate at the quantum scale top-down (Phases 27--35); (viii) test and honestly bound the strong-coupling and QM claims (Phases 36--42); (ix) prove the $\alpha_s$ closure reference-irreducible and test the crystallization ruler (Phases 43--44); (x) derive the twist, the Standard Model counting, and the mass-ratio duality (Phases 45, 47--49); (xi) draw the precise boundary for where $\varphi$ lives (Phases 50--51).
...
**The $\alpha_s$ closure is reference-irreducible** (Phases 43, 46). Phase 43 found that the apparent two-loop resolution of the $m_b$ residual was an artifact: the coupling's second coefficient in the golden scheme was dead code ($0.0 \times k_1$). With the real two-loop coefficient, $m_b$ closes (+15.95%→+0.75%) but $M_Z$ and $m_t$ are over-corrected (−42%, −76%); overlaying the full-curve two-loop MS-bar RGE shows an irreducible conflict in the $m_b \to M_Z$ segment (+31.5% too steep). Phase 46 then tested *every legitimate reference choice* -- the scheme-dependent $m_t$ (0.090 vs the QCD-running 0.108), the QCD-consistent reference set (0.3133/0.2236/0.1180/0.1076), and fully free references within credible ranges. **None closes the closure.** The structural cause is a shape mismatch: matching two-loop QCD in the $m_b \to M_Z$ segment requires a golden exponent of $\varphi^{+0.82}$ (flattening), the *opposite sign* of the principled $\varphi^{-(n_f-3)/6}$ (steepening). Golden running is a pure power law in $E$; QCD running is $\sim 1/\ln E$ and flattens at high energy. The flavor-threshold mechanism is confirmed; the clean golden closure is closed with a definite, reference-independent negative.

**The BAO sound-horizon test** (Phase 44). Phase 36's dimensional crystallization (D → 2 before recombination) was confronted with the DESI DR1 BAO standard ruler ($D_M/r_d$ and $D_H/r_d$ at $z = 0.51$–$1.49$, 1–5% precision, $r_d = 147.09$ Mpc). The joint H(z)+BAO fit is $\Delta\chi^2 = -4.6$ (no discrimination); the BAO-only $z_c$ basin is flat ($\chi^2$ 35–38 across $z_c = 0.5$–8), so the ruler cannot pin the crystallization redshift at $z \le 1.5$. The ruling tension is the known low-$z$ DESI $D_H(0.51)$ anomaly, which strikes both models equally (+5.7σ crystallization, +5.6σ ΛCDM). **Honest conclusion:** the BAO ruler confirms Phase 36 -- the refined picture (crystallization before recombination, D ≈ 3 at all observable $z$) survives the standard ruler, and the discriminators remain at higher $z$.
...
### 5.7 The True Incommensurate Spectrum (Phase 51)

Phase 1's falsification of a static-$\varphi$ invariant in the substrate Laplacian was performed on a *commensurate* (rational) raster grid whose spectral circle carries the number-theoretic $4p^2 + \ell^2$ ladder and unavoidable mode-locking. Phase 51 rebuilds the analysis on the *incommensurate* golden-angle (Fibonacci) lattice prescribed by the substrate's discreteness constraint, keeping the Klein non-orientability as a global parity condition.

**H51a -- the 1D Fibonacci chain is exactly solvable (Kohmoto–Kadanoff–Tang).** The transfer-matrix traces over the Fibonacci word satisfy the three-term map $x_{n+1} = 2x_n x_{n-1} - x_{n-2}$ to 2e-13 (machine precision), and the KKT (Fricke) invariant $x_{n+1}^2 + x_n^2 + x_{n-1}^2 - 2x_{n+1}x_n x_{n-1}$ is conserved to 5e-10 -- a *provably exact* golden self-similarity in the static spectrum. The spectrum fragments as a **Cantor set**: 359 distinct bands at generation 14, while the periodic (rational) control stays at 2 bands. This is the incommensurate anti-resonant gap structure -- the static spectral analogue of Phase 6's dynamical gap-rigidity persistence.
...
**H51c -- spectral RG honest negative.** Coarse-graining the 2D Laplacian by Galerkin projection onto the low-energy eigenspace keeps $D_{\text{eff}} \approx 2.2$ ($r^2 \approx 0.995$) across all scales -- **never $\varphi$**. Even on the true incommensurate lattice, $\varphi$ is not a static spectral dimension.

**Honest conclusion.** Phase 51 settles the "was Phase 1's negative a raster artifact?" question with a refined negative: the raster grid did hide the incommensurate gap structure, but the true lattice still does not make $D_{\text{eff}} = \varphi$. The golden structure lives in the Cantor gap hierarchy (H51a) and the topological twist fraction (H51b), not in the spectral dimension -- completing the picture Phases 37, 46, and 50 drew: $\varphi$ lives in the structure that survives -- the fractal gap hierarchy and the parity -- not in any single static observable.

## 6. Quantum-Scale Derivations: The Half-Integer Twist and the Particle Spectrum

The framework's most secure quantitative claims are top-down ratio tests at the quantum scale (Phases 27--35), all traced to the single half-integer twist $\theta = 1/2$. Phases 47--49 complete this arc by *deriving* the twist from the substrate's topology and tracing the Standard Model's counting structure and the mass-ratio duality to the same Fibonacci lattice.

### 6.1 The Half-Integer Twist (Phases 25, 29, 47)
...
**Phase 47 -- the derivation.** Phases 28--35 used $\theta = 1/2$ as an empirical constant; Phase 47 closes the foundational gap by deriving it. The non-orientable seam is a flat $\mathbb{Z}_2$ gauge connection with meridian holonomy $W = -1$. But the master equation's associator term -- and quantum amplitudes generally -- require a complex Hilbert space, so the substrate's real line bundle must be *embedded* into a complex $U(1)$ bundle, mapping the $\mathbb{Z}_2$ holonomy $-1$ to the phase $e^{i\pi}$. The fractional topological charge (the twist) is defined by the $U(1)$ winding number,
$$\theta = \frac{\arg(W)}{2\pi} = \frac{\pi}{2\pi} = \frac{1}{2},$$
exactly and parameter-free. Computing the discrete Wilson loop on graph sizes from $3 \times 3$ to $144 \times 233$ confirms $\theta = 0.5$ is exact and scale-invariant -- a property of the topology, not the discretization. This connects directly to the $SU(2)$ double-cover: a full $720^{\circ}$ rotation gives $+I$ while a single $360^{\circ}$ traversal gives $-I$; in the $U(1)$ embedding that single traversal is $W = -1$, exactly a half-rotation in phase space. **The fractional twist is no longer empirical; it is a derived topological invariant of the Klein seam under complex embedding.**
...
**Phase 49 -- the factor 6 is derived.** The "6" in $6\pi^5$ was the one remaining empirical input. In algebraic topology the homological (Poincaré) volume of a compact Lie group is the product of the volumes of its generating spheres; for $SU(3)$ the generators are $S^3$ and $S^5$, giving exactly ${\rm Vol}_{\text{topo}}(SU(3)) = 2\pi^5$. The ratio then factors as
$$\frac{m_p}{m_e} = 6\pi^5 = 3 \times (2\pi^5) = N_c \times {\rm Vol}_{\text{topo}}(SU(3)),$$
with $N_c = 3$ the number of quark colors in a baryon (the Phase 48 $F_4$). The physical content is a duality between confinement and the free lepton: the three confined quarks' $SU(3)$ color degrees of freedom are restricted to a color-singlet; the electron, an $SU(3)$ singlet, avoids this confinement, and its relative phase-space volume exactly equals the unconstrained $SU(3)$ phase space "missing" from the three quarks. The last empirical constant in the mass formulas is removed by topology.
...
**Honest scope.** The decuplet is the clean object (≤0.29%); the octet does NOT fit a single ladder -- its law is the golden partition, resolved in Phase 45 (Section 6.8). The $\Delta$ anchor and the 1-fm scale retain a combined ~1% ambiguity.
...
### 6.8 The Baryon Octet: The Golden Partition (Phase 45)
...
The two internal gaps -- the $\Lambda{-}\Sigma$ hyperfine split (ud spin-flip, I=0↔I=1) and the $\Xi{-}\Sigma$ strangeness step (S=−1→−2) -- stand in the golden ratio. The relation passes the project's robustness frame: the split fraction $1/\varphi^2$ has a narrow 0.38% basin with $1/\varphi^2$ uniquely selected against competitors (3/8, 0.38, 5/13, 8/21, 0.39, 0.4). The octet obeys the Gell-Mann–Okubo sum rule to 0.57% as a consistency check. The framework now has two clean, parameter-free SU(3) mass structures: the decuplet's E-ladder and the octet's golden partition. The honest scope: this is a single parameter-free constraint on three measured masses, characterizing the residual structure GMO leaves open -- it does not replace GMO at leading order.

### 6.9 The Fibonacci Standard Model (Phase 48)

Phase 24's parameter scan established that ~3% of substrate nodes form stable topological defects (knots), robust to all dynamic variations. Phase 48 maps this stable-knot fraction to the Standard Model's particle multiplicities, resolving the final structural open item. Because the substrate is cellulated with a Fibonacci lattice, the allowable topological defects are constrained by the Fibonacci sequence $F_n = \{1, 1, 2, 3, 5, 8, 13, 21, 34, \ldots\}$, and the SM's entire fundamental counting structure maps to its first nine terms:

- **$F_1 = 1$:** the Higgs boson.
- **$F_2 = 1$:** the photon ($U(1)$ gauge boson).
- **$F_3 = 2$:** the chiralities (left, right).
...
**The $1/34$ knot fraction.** The theoretical probability of a node forming a stable knot is exactly $1/F_9 = 1/34 \approx 2.941\%$, statistically consistent with the Phase 24 empirical mean ($3.132\% \pm 0.483\%$). The empirical "~3%" is the $F_9$ structural limit. **The boson/fermion ratio.** The total bosons ($F_7 = 13$) to fermions per generation ($F_6 = 8$) ratio is $13/8 = 1.625$, the standard Fibonacci approximation of $\varphi$. This parameter-free combinatorial map derives the SM's structural quantities from the Fibonacci sequence inherent to the substrate, without new assumptions.

### 6.10 The Light-Quark Golden Partition Test (Phase 50): Where φ Does NOT Live

Phase 45 showed the baryon octet (bound states of light quarks) obeys the golden partition; Phase 50 asks whether the *bare* quarks ($u, d, s$) themselves carry it. The answer is a decisive, RG-invariant honest negative.

**The bare partition fails.** If the quarks carried the octet's law, they would satisfy $(m_d - m_u)/(m_s - m_u) = 1/\varphi^2 \approx 0.382$. The measured bare masses ($m_u = 2.16$, $m_d = 4.67$, $m_s = 93.4$ MeV) give $0.0275$ -- 92.8% off. **The failure is scale-invariant:** to one loop all light quarks share the same anomalous dimension $\gamma_m$, so their *mass ratios* are exactly RG-invariant -- running all three masses by any common factor leaves the ratio identical. The bare quarks do not golden-partition at *any* scale. **It is not a coordinate artifact:** the partition fails in the Koide $\sqrt{m}$ space too (0.084 vs 0.382).

**Refined conclusion.** The golden partition is a structural law of the hadronic *bound states* -- the topological knots that are the physical particles -- not of the perturbative, scheme-dependent bare quarks. This is the same dividing line Phase 37 drew (golden harmonics live in the masses, not the couplings) and Phase 46 reinforced (the golden power-law cannot reproduce bare QCD running). $\varphi$ is a property of the *emergent, confined* substrate excitations.
...
The arc across 51 phases converges on a single picture: $\varphi$ is not written into the substrate's spatial structure. It emerges from the temporal dynamics of harmonic self-interaction through three interconnected mechanisms -- anti-resonance selection, the vacuum-pump laser threshold, and dynamical RG convergence -- with fold-density feedback closing the loop so that $G_{\text{eff}}$ converges to $\rho^{1/\varphi}$ from any initial condition. The later phases add the crucial refinement of *where* $\varphi$ does and does not live: it is a dynamical attractor in the time domain, an exact self-similarity and parity in the static spectrum, and a structural law of bound-state knots -- never a static spectral dimension, a bare coupling, or a bare-quark mass (Sections 5.5, 5.7, 6.8, 6.10).
...
- **The $\alpha_s$ closure is reference-irreducible (Phases 43, 46).** The apparent two-loop "closure" of the $m_b$ residual was a dead-code artifact; with the real two-loop coefficient no single golden rule fits all four references, because the golden power-law running cannot reproduce the genuine $\sim 1/\ln E$ curvature of QCD. The negative is reference-independent (Sections 5.5).
- **The BAO ruler does not discriminate crystallization (Phase 44).** The DESI DR1 sound-horizon ruler is flat against the crystallization redshift and adds no discriminating power at $z \le 1.5$ — an honest negative that confirms Phase 36 (Section 5.5).
- **Bare quarks do not golden-partition (Phase 50).** The bare mass ratio fails $1/\varphi^2$ by 92.8%, RG-invariantly and in Koide space — the partition is a law of bound-state knots (Section 6.10).
- **$\varphi$ is not a static spectral dimension (Phase 51).** Even on the true incommensurate Fibonacci lattice, spectral RG keeps $D_{\text{eff}} \approx 2.2$ — never $\varphi$; the golden structure lives in the Cantor gap hierarchy and the twist (Section 5.7).
...
The stable-knot fraction has been mapped to the Standard Model multiplicities (Phase 48); the clean golden closure of the $\alpha_s$ flavor running is *closed* as reference-irreducible (Phases 43, 46); and the octet's structure is resolved as a golden partition (Phase 45). New open questions remain: (i) deriving the octet's two SU(3) laws (decuplet E-ladder, octet golden partition) from a single $\theta = 1/2$ mechanism; (ii) whether a golden-informed *modification* of QCD running -- rather than a replacement -- can close the $\alpha_s$ gap; and (iii) the exact origin of the incommensurate spectrum's $D_{\text{eff}} \approx 2.2$. The $\varphi$-attractor's connection to established quantum-gravity frameworks (LQG, asymptotic safety) remains to be formalized.

## 9. Conclusion

Information Substrate Theory provides a framework in which observed physics emerges from a discrete, non-orientable two-dimensional information substrate. A 51-phase computational program (600 automated tests) establishes that the golden ratio $\varphi$ is not a static invariant of the substrate graph but a **dynamical attractor** of its harmonic self-interaction.
...
| 34 | Baryon mass ladder | Decuplet ≤0.27%; octet honest (mixing open, closed Ph 45) |
...
| 43 | 2-loop golden closure | Dead-code b1 fixed: $m_b$ closes (+15.95%→+0.75%) but the two-loop RGE overlay is IRREDUCIBLE — no single golden rule closes all 4 |
| 44 | BAO sound-horizon test | DESI DR1 ruler flat against crystallization ($\Delta\chi^2=-4.6$); CONFIRMS Phase 36 — discriminators remain at higher z |
| 45 | Baryon octet golden partition | $\Sigma$ splits $\Lambda\to\Xi$ at $1/\varphi^2$ (0.108%); parameter-free $\Sigma\to$0.007%, $\Xi\to$0.017% — two SU(3) laws |
| 46 | Reference-level fix refuted | All four reference choices leave $m_b/M_Z$ OUT; closure is reference-irreducible (power-law-vs-log shape mismatch) |
| 47 | Emergent-twist derivation | $\theta = 1/2$ derived exactly from the $\mathbb{Z}_2\to U(1)$ holonomy embedding ($\theta = \arg(W)/2\pi$); grid-independent |
| 48 | Stable-knot SM multiplicities | SM counting = $F_1$…$F_9$; knot fraction exactly $1/34 \approx 2.94\%$; boson/fermion $13/8 \approx \varphi$ |
| 49 | Proton/electron ratio derived | $m_p/m_e = N_c\,{\rm Vol}(SU(3)) = 6\pi^5$ — the empirical 6 is the confined-color phase-space duality |
| 50 | Light-quark golden partition test | Bare quarks fail $1/\varphi^2$ by 92.8%, RG-invariantly — partition is a law of bound-state knots, not bare quarks |
| 51 | Fibonacci Laplacian | True incommensurate spectrum: EXACT KKT trace map (2e-13) + Cantor fragmentation (359 vs 2 bands); twist 0.446 N-independent; $D_{\text{eff}}\approx2.2$ NEVER $\varphi$ |
