...
**Anthropological note (genealogy, not evidence).** The architecture of
thought this note develops — an undifferentiated zero, oscillation as the
first event, complementary dual directions of one motion, spiral/layered
condensation of reality — recurs in non-physics traditions (most closely the
seven-stage logarithmic-spiral cosmology of Michio Kushi's *Order of the
Universe*, and behind it Taoist cosmology). This is recorded here as a
genealogical prior only: those systems are unfalsifiable and make no
quantitative predictions. IST stands on θ = ½ and the runtime alone, and the
physics must be judged on those grounds.
...
events. Phase 13's dynamical RG produces D_eff flowing under temporal
evolution to ≈ φ (1.655, within 2.3%) — a spectral dimension *changing* as
coherent structure emerges, exactly the P2 coherence-threshold logic.
Phase 14's fold-density feedback ODE `df/dt = γ(D_eff(f) − φ)f` drives any
initial condition onto the golden window (f ≈ 4.2) where D_eff = φ — a
self-regulating crystallization process. Neither simulates stacking; both
demonstrate that dimension is an outcome of dynamics in this framework, not
a fixed input. They calibrate the "Phase 6x" stacking automaton (§9).
...
- Whitney immersion/embedding theorems; the classical unknotting theorem
  in codimension ≥ 3. For the surface-knot sector of §7.2: Kamada,
  *Surface-Knots in 4-Space* (2017); Hillman (2002).
...
**Strand-count ↔ dimension-occupancy rule (classifier for future work).**
The two seam geometries have a systematic read: a *dual-strand* structure
(photon, rungs crossing Ω) uses the cross-Ω stacking direction and is
achiral (parity-inversion 0.000); a *single-strand* structure (electron
knot) lives in-sheet, threads the seam, and carries seam parity (0.446).
...
**7.2 Unknotting = disintegration of matter — with a survivor class.**
By the theorem of §4, in the locally 4D interior all *thread*-knots untie.
Matter does not survive as configured structure. This is the IST mechanism
behind the *appearance* of a singularity: the interior is not a point of
infinite density but a region where information has returned to the
unconfigured, zero-point state of P0 — a **macroscopic projection of the Ω
locus**. The information is all in there, but it is no longer knotted into
matter: it is pure probabilistic information, an undifferentiated
superposition, until release.

**Refinement (Kamada).** The disintegration arithmetic is overdetermined
but not total. In the local-4D interior, particle-knots (1D objects in 3D,
now codimension-3) unknot *faster* than the §4 baseline. But by the
classical theory of surface-knots, *2-knots* (knotted 2D surfaces) are the
stable codimension-2 objects in 4-manifolds (Kamada, *Surface-Knots in
4-Space*, 2017; Hillman 2002). So §7.2 should read: **particle-knots
disintegrate; surface-knotted excitations may persist.** This has a direct
consequence for the release channel: the Hawking knot-census spectrum of
§7.4 may need a surface-knot sector in addition to the thread census — a
distinct, identifiable family of spectral lines that standard thermal
Hawking radiation lacks.
...
- Horizon hysteresis memory (2667 flips, Run A): reversible topology
  ...
  fluctuations.
- Surface-knot sector in the release spectrum (new, §7.2): if the interior
  preserves 2-knots, the Hawking knot-census should show a second family of
  peaks beyond the Lk-indexed thread lines.
...
6. **Bidirectional parthood (philosophical framing).** In the taxonomy of
   mereological spacetime-emergence models, IST is distinctive in being
   *bidirectional*: below the coherence threshold, P0's whole is prior to
   its parts (priority-monism); above it, the stack is posterior to its
   parts (P3, composition). The direction of parthood flips at the
...
3. §7 → integration with the BH topology chapter (Phase 2) and the
   information-paradox discussion; sharpen the three falsifiable hooks and
   the new surface-knot sector.
4. Only then: a computational phase (tentative "Phase 6x: sheet-stacking
   ...
   Calibration targets already exist: Phase 13 (D_eff flow to φ) and
   Phase 14 (fold-feedback pinning) fix the single-sheet behavior the
   stacking must build on.
